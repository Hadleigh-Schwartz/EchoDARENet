...
class CepstralDomainDecodingLoss(DecodingLoss):
    def __init__(self, delays, win_size, decoding, cutoff_freq, sample_rate, softargmax_beta=1e10):
        super(CepstralDomainDecodingLoss, self).__init__(delays, win_size, "cepstrum", cutoff_freq, sample_rate, softargmax_beta)
    
    def compute_symbol(self, cepstrum):
        """
        Compute the symbol for a given cepstrum.

        Parameters:
            cepstrum : torch.Tensor (win_size // 2)
                One audio window's one-sided cepstrum 
    ...
    def forward(self, cepstrum_batch, gt_symbols_batch, num_errs_no_reverb_batch, num_errs_reverb_batch):
        """
        Compute various loss functions for the decoding task.

        Parameters:
            cepstrum_batch : torch.Tensor (batch_size, 2, win_size // 2, num_wins)
                Batch of complex cepstra for all windows in each audio sample
            gt_symbols_batch : torch.Tensor (batch_size, num_symbols)
                Batch of ground-truth symbols that were encoded onto the clean speech
        ...
        Return:
            sym_err_rate : float
                Symbol error rate for the decoding task
            avg_err_reduction_loss : float
                Average error reduction loss for the decoding task
            no_reverb_sym_err_rate : float
                Symbol error rate for the non-reverberant audio
            reverb_sym_err_rate : float
                Symbol error rate for the reverberant audio
        """
        num_wins = cepstrum_batch.shape[3] 
        num_batch_samples = cepstrum_batch.shape[0]

        cepstrum_batch = cepstrum_batch[:, 0, :, :].squeeze(1) # (batch_size, 2, win_size // 2, num_wins) -> (batch_size, win_size // 2, num_wins)
        cepstrum_batch = cepstrum_batch.permute(0, 2, 1) # (batch_size, win_size // 2, num_wins) -> (batch_size, num_wins, win_size // 2)
        cepstrum_batch = cepstrum_batch.reshape(cepstrum_batch.shape[0] * cepstrum_batch.shape[1], self.win_size // 2) # (batch_size, num_wins, win_size // 2) -> (batch_size * num_wins, win_size // 2)

        # compute symbols for all windows 
        all_pred_symbols = torch.vmap(self.compute_symbol)(cepstrum_batch) # apply compute_symbol to each window in all_windows
        ...
        all_pred_symbols = all_pred_symbols.reshape(num_batch_samples, num_wins) # (batch_size * num_wins, 1) -> (batch_size, num_wins)
        samplewise_accuracy = Accuracy(task="multiclass", num_classes=len(self.delays), multidim_average='samplewise').to(cepstrum_batch.device) 
        ...
        no_reverb_sym_err_rate = torch.sum(num_errs_no_reverb_batch) / (num_batch_samples * num_wins)
        reverb_sym_err_rate = torch.sum(num_errs_reverb_batch) / (num_batch_samples * num_wins)
