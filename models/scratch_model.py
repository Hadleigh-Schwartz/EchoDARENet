...
class Test(pl.LightningModule):
    ...
    def init(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(  2,  64, k, stride=s, padding=(k+1, k//2), dilation = (3, 1)), nn.LeakyReLU(leaky_slope))
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(512, 512, k, stride=1, padding=k//2), nn.BatchNorm2d(512), nn.LeakyReLU(leaky_slope))
        self.conv6 = nn.Sequential(nn.Conv2d(512, 512, k, stride=s, padding=k//2), nn.BatchNorm2d(512), nn.ReLU())
        ...
        self.deconv2 = nn.Sequential(nn.ConvTranspose2d(1024, 512, k, stride=1, padding=k//2, output_padding=0), nn.BatchNorm2d(512), nn.Dropout2d(p=p_drop), nn.ReLU())
        self.deconv3 = nn.Sequential(nn.ConvTranspose2d(1024, 256, k, stride=s, padding=k//2, output_padding=s-1), nn.BatchNorm2d(256), nn.Dropout2d(p=p_drop), nn.ReLU())
        self.deconv4 = nn.Sequential(nn.ConvTranspose2d(512,  128, k, stride=s, padding=k//2, output_padding=s-1), nn.BatchNorm2d(128),  nn.Dropout2d(p=p_drop), nn.ReLU())
        self.deconv5 = nn.Sequential(nn.ConvTranspose2d(256,   64, k, stride=s, padding=k//2, output_padding=s-1), nn.BatchNorm2d(64), nn.ReLU()) # important to have Tanh not previous version's ReLU otherwise can't be negative
        self.deconv6 = nn.Sequential(nn.ConvTranspose2d(128,   2, k, stride=s, padding=(k+1, k//2), output_padding=s-1, dilation=(3, 1)),  nn.BatchNorm2d(2), nn.Tanh()) # important to have Tanh not previous version's ReLU otherwise can't be negative
    ...
    def predict(self, x):
        ...
        c1Out = self.conv1(x)     
        print("c1Out shape: ", c1Out.shape)
        c2Out = self.conv2(c1Out) 
        print("c2Out shape: ", c2Out.shape)
        c3Out = self.conv3(c2Out) 
        print("c3Out shape: ", c3Out.shape)
        c4Out = self.conv4(c3Out) 
        print("c4Out shape: ", c4Out.shape)
        c5Out = self.conv5(c4Out) 
        print("c5Out shape: ", c5Out.shape)
        c6Out = self.conv6(c5Out)
        print("c6Out shape: ", c6Out.shape)
    
        if self.use_transformer:
            c6Out = self.transformer(\
                c6Out.squeeze().permute((0,2,1)), \
                c6Out.squeeze().permute((0,2,1))).permute((0,2,1)).unsqueeze(-1)
        
        print("C6 out post transformer shape: ", c6Out.shape)

        d1Out = self.deconv1(c6Out) 
        print("D1 out shape: ", d1Out.shape)
        d2Out = self.deconv2(t.cat((d1Out, c5Out), dim=1)) 
        print("D2 out shape: ", d2Out.shape)
        d3Out = self.deconv3(t.cat((d2Out, c4Out), dim=1)) 
        print("D3 out shape: ", d3Out.shape)
        d4Out = self.deconv4(t.cat((d3Out, c3Out), dim=1)) 
        print("D4 out shape: ", d4Out.shape)
        d5Out = self.deconv5(t.cat((d4Out, c2Out), dim=1)) 
        print("D5 out shape: ", d5Out.shape)
        d6Out = self.deconv6(t.cat((d5Out, c1Out), dim=1))
        print("D6 out shape: ", d6Out.shape)
   
        if self.residual:
            cep_out = d6Out + residual
        else:
            cep_out = d6Out
