...
class DetmoldIRDataset(Dataset):
    def __init__(self, config, type="train", split_train_val_test_p=[80, 10, 10], device='cuda'):
        ...
        all_filenames = sorted(glob.glob(os.path.join(dir, "SetA_SingleSources", "Data", "**", "*.wav"), recursive=True))

        self.max_data_len = len(all_filenames)
        if type == "train" and "detmold" not in config.val_rir_datasets and "detmold" not in config.test_rir_datasets:
            # all idxs can be used for training, no need to split
            split_idxs = list(range(self.max_data_len))
        elif type == "val" and "detmold"  not in config.train_rir_datasets and "detmold"  not in config.test_rir_datasets:
            # all idxs can be used for validation, no need to split
            split_idxs = list(range(self.max_data_len))
        elif type == "test" and "detmold"  not in config.train_rir_datasets and "detmold"  not in config.val_rir_datasets:
        ...
        print(self.max_data_len, "total RIRs in detmold dataset")
...
def detmoldIRDataloader(config_path, type="train"):
    return DataLoader(DetmoldIRDataset(config_path, type=type))
