...
class BRUDEXIRDataset(Dataset):
    def __init__(self, config, type="train", split_train_val_test_p=[80, 10, 10], device='cuda'):
        ...
        if type == "train" and "BRUDEX" not in config.train_rir_datasets:
            print("WARNING: Loading BRUDEX dataset for training, but BRUDEX is not in the config's training rir datasets list. This may lead to unexpected results.")
        elif type == "val" and "BRUDEX" not in config.val_rir_datasets:
            print("WARNING: Loading BRUDEX dataset for validation, but BRUDEX is not in the config's validation rir datasets list. This may lead to unexpected results.")
        elif type == "test" and "BRUDEX" not in config.test_rir_datasets:
            print("WARNING: Loading BRUDEX dataset for testing, but BRUDEX is not in the config's testing rir datasets list. This may lead to unexpected results.")
        
        dir = os.path.join(self.root_dir, "BRUDEX")
        all_filenames = sorted(glob.glob(os.path.join(dir, "rir", "**", "*.mat"), recursive=True))

        self.max_data_len = len(all_filenames)
        if type == "train" and "BRUDEX" not in config.val_rir_datasets and "BRUDEX" not in config.test_rir_datasets:
            # all idxs can be used for training, no need to split
            split_idxs = list(range(self.max_data_len))
        elif type == "val" and "BRUDEX"  not in config.train_rir_datasets and "BRUDEX"  not in config.test_rir_datasets:
            # all idxs can be used for validation, no need to split
            split_idxs = list(range(self.max_data_len))
        elif type == "test" and "BRUDEX"  not in config.train_rir_datasets and "BRUDEX"  not in config.val_rir_datasets:
        ...
        print(self.max_data_len, "total RIRs in BRUDEX dataset")
    ...
    def get_exclude_rirs(self):
        ...
        for idx in range(len(self.split_filenames)):
            ...
            rir, sr = self.read_mat_file(filename)
    ...
    def __getitem__(self, idx):
        ...
        rir, sr = self.read_mat_file(path)
        filename = os.path.basename(path)
        return rir, filename

 
def BRUDEXIRDataloader(config_path, type="train"):
    return DataLoader(BRUDEXIRDataset(config_path, type=type))